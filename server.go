...
func (r *Server) Shutdown(reason string) error {
	log.Printf("shutting down RabbitMQ connection to %s. Reason: %s", r.url, reason)
...
func (r *Server) notifyHandler(notify chan *amqp.Error) {
	for {
		select {
		case err := <-notify:
			log.Printf("received NotifyClose with error value %v", err)
...
func (r *Server) reconnect() error {
	...
	time.Sleep(time.Duration(r.reconnAttempt*10) * time.Second)
	if err := r.connect(); err != nil {
		log.Printf(`couldn't reconnect to RabbitMQ server.Address:	%s Attempt:	%d Error: %v`, r.url, r.reconnAttempt, err)
		go r.sendErrors(err)
		r.reconnect()
	}
